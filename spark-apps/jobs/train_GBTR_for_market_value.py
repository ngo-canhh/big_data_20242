# --- START OF FILE train_player_model_gbt.py ---
...
# Đổi tên thư mục lưu mô hình để phân biệt với các mô hình trước đó
MODEL_PATH = os.getenv("MODEL_PATH", "hdfs://namenode-h2dn:9000/user/models/player_market_value_prediction_model_gbt")

# Định nghĩa cột mục tiêu (Label)
LABEL_COL = "market_value"

def main():
    logger.info(f"Starting Spark job to train {LABEL_COL} prediction model for player data using GBTRegressor")

    # Khởi tạo Spark Session
    logger.info("Initializing SparkSession...")
    spark = SparkSession.builder \
        .appName(f"TrainPlayerModelGBT_{LABEL_COL}") \
    ...
    try:
        ...
        # Lưu ý: GBT ít nhạy cảm với scale hơn Linear Regression,
        ...
        gbt = GBTRegressor(
            ...
            maxIter=20,                  # Hyperparameter: Số lượng cây (iterations) (có thể tinh chỉnh)
            maxDepth=5,                  # Hyperparameter: Độ sâu tối đa của mỗi cây (có thể tinh chỉnh)
            stepSize=0.1,                # Hyperparameter: Learning rate (có thể tinh chỉnh)
            seed=42                      # Seed để đảm bảo kết quả huấn luyện nhất quán
        )
        logger.info(f"Training GBT Regressor model for {LABEL_COL}...")
        # Fit mô hình trên tập huấn luyện
        gbt_model = gbt.fit(train_df)
        logger.info("GBT Regressor model training complete.")


        # --- Đánh giá mô hình trên tập kiểm tra ---
        logger.info(f"Evaluating model on test set...")
        # Áp dụng mô hình đã fit lên tập kiểm tra để có các dự đoán
        predictions = gbt_model.transform(test_df) # Sử dụng gbt_model
        ...
        # --- Lưu mô hình ---
        # Lưu pipeline model tiền xử lý và mô hình GBT đã huấn luyện
        # Việc lưu pipeline model giúp áp dụng cùng tiền xử lý cho dữ liệu mới khi dự đoán
        logger.info(f"Saving pipeline model and trained GBT model to {MODEL_PATH}")

        # Lưu mô hình GBTRegressor đã fit
        gbt_model_save_path = os.path.join(MODEL_PATH, "gbt_model") # Sử dụng thư mục con "gbt_model"
        logger.info(f"Saving GBT model to {gbt_model_save_path}")
        gbt_model.write().overwrite().save(gbt_model_save_path) # Sử dụng gbt_model.write().overwrite().save
        logger.info(f"GBT model saved successfully.")
...
if __name__ == "__main__":
    main()
# --- END OF FILE train_player_model_gbt.py ---